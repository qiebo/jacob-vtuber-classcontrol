...
class ServiceContext:
    ...
    def __init__(self):
        ...
        # Session-level toggle: whether conversation should inject retrieved KB context.
        self.knowledge_enabled: bool = False

        self.classroom_username: str | None = None
        self.classroom_class_name: str | None = None
        self.classroom_dirty: bool = False
        self.classroom_submitted: bool = False
        self.classroom_last_saved_at: str | None = None
        self.classroom_locked: bool = False
    ...
    async def apply_character_config(
        self,
        character_config: CharacterConfig,
        reinitialize_runtime: bool = True,
    ) -> None:
        """Apply a classroom-loaded character config to this context."""
        previous_character_config = self.character_config
        previous_agent_engine = self.agent_engine
        target_character_config = character_config.model_copy(deep=True)
        target_persona_prompt = target_character_config.persona_prompt
        previous_prompt = (
            previous_character_config.persona_prompt
            if previous_character_config and previous_character_config.persona_prompt
            else target_persona_prompt
        )

        self.character_config = target_character_config
        self.character_config.persona_prompt = previous_prompt
        if self.config:
            self.config.character_config = self.character_config

        if reinitialize_runtime:
            self.init_live2d(target_character_config.live2d_model_name)
            # init_agent skips work when the incoming config equals the current one.
            # During classroom profile load we have already assigned the target
            # config, so compare against the previous session config instead.
            if (
                previous_agent_engine is not None
                and previous_character_config is not None
                and target_character_config.agent_config
                == previous_character_config.agent_config
                and target_persona_prompt == previous_prompt
            ):
                self.agent_engine = previous_agent_engine
            else:
                self.agent_engine = None
            await self.init_agent(
                target_character_config.agent_config,
                target_persona_prompt,
            )

        self.character_config.persona_prompt = target_persona_prompt
        if self.config:
            self.config.character_config = self.character_config
                    ...
                    json.dumps(
                        {
                            ...
                            "avatar_mode": self.character_config.avatar_mode,
                            "avatar_pack_id": self.character_config.avatar_pack_id,
                        }
                    )
                )
