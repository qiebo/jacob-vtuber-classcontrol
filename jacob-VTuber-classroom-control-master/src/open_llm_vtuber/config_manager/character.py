...
class CharacterConfig(I18nMixin):
    ...
    avatar_mode: str = Field(default="live2d", alias="avatar_mode")
    avatar_pack_id: str = Field(default="", alias="avatar_pack_id")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "avatar_mode": Description(
            en="Avatar rendering mode: live2d or avatarpack",
            zh="形象渲染模式：live2d 或 avatarpack",
        ),
        "avatar_pack_id": Description(
            en="Active avatar pack id when avatar_mode=avatarpack",
            zh="当 avatar_mode=avatarpack 时启用的人物包 ID",
        ),
    ...
    def check_default_persona_prompt(cls, v):
        ...
        return ensure_teaching_persona_safety(v)
    ...
    def validate_avatar_mode(cls, value: str):
        normalized = (value or "live2d").strip().lower()
        if normalized not in {"live2d", "avatarpack"}:
            raise ValueError("avatar_mode must be either 'live2d' or 'avatarpack'")
        return normalized
