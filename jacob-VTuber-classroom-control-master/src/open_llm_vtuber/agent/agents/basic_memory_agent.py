...
MAX_MEMORY_MESSAGES_FOR_PROMPT = 8
...
class BasicMemoryAgent(AgentInterface):
    ...
    def _prompt_memory(self) -> List[Dict[str, Any]]:
        """Return a bounded memory window for the next LLM request."""
        if len(self._memory) <= MAX_MEMORY_MESSAGES_FOR_PROMPT:
            return self._memory.copy()

        trimmed_memory = self._memory[-MAX_MEMORY_MESSAGES_FOR_PROMPT:].copy()
        while trimmed_memory and trimmed_memory[0].get("role") == "assistant":
            trimmed_memory.pop(0)

        logger.info(
            "Trimmed LLM memory context from "
            f"{len(self._memory)} to {len(trimmed_memory)} messages."
        )
        return trimmed_memory
        ...
        if isinstance(content, list):
            ...
            for item in content:
                if isinstance(item, dict):
                    total += len(str(item.get("text") or item.get("type") or ""))
                else:
                    total += len(str(item))
            return total
    ...
    def _to_messages(self, input_data: BatchInput) -> List[Dict[str, Any]]:
        ...
        messages = self._prompt_memory()
        metadata = input_data.metadata or {}

        knowledge_context = str(metadata.get("knowledge_context") or "").strip()
        if knowledge_context:
            messages.append(
                {
                    "role": "system",
                    "content": (
                        "以下是从用户上传知识库中检索到的参考片段。"
                        "仅在问题相关时引用；若不相关请忽略。"
                        "不得编造片段中不存在的事实。\n\n"
                        f"{knowledge_context}"
                    ),
        ...
        if user_content:
            ...
            skip_memory = bool(metadata.get("skip_memory", False))
    ...
    def _chat_function_factory(
        ...
        async def chat_with_memory(
            ...
            prompt_chars = sum(self._message_content_length(message) for message in messages)
            logger.info(
                "Prepared LLM prompt with "
                f"{len(messages)} message(s), approx_chars={prompt_chars}."
            )
            tools = None
            tool_mode = None
                ...
                async for event in token_stream:
                    ...
                    if text_chunk:
                        if not first_token_logged:
                            ...
                            logger.info(
                                "LLM first token after "
                                f"{time.perf_counter() - llm_start_time:.2f}s."
                            )
                        yield text_chunk
                        complete_response += text_chunk
                logger.info(
                    "LLM stream completed in "
                    f"{time.perf_counter() - llm_start_time:.2f}s."
                )
