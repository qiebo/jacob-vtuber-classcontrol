...
class AsyncLLM(StatelessLLMInterface):
    def __init__(
        ...
        self.extra_body = NOT_GIVEN
        if "dashscope" in normalized_base_url and normalized_model.startswith("qwen"):
        ...
        try:
            ...
            for message in messages_with_system:
                ...
                if isinstance(content, str):
                    ...
                    message_chars += sum(len(str(item)) for item in content)
                else:
                    message_chars += len(str(content))
            logger.debug(
                "Messages prepared for OpenAI-compatible LLM: "
                f"count={len(messages_with_system)}, approx_chars={message_chars}"
            )
