...
class TTSInterface(metaclass=abc.ABCMeta):
    ...
    def supports_frontend_streaming(self) -> bool:
        """
        Whether this TTS engine should use frontend streaming payload mode.

        Default is False to keep backwards compatibility.
        """
        return False

    def max_concurrent_generations(self) -> int:
        """
        Maximum number of audio generation requests this engine can handle in parallel.

        Cloud providers often enforce low per-token concurrency limits. Engines can
        override this to prevent silent audio drops caused by provider-side 429s.
        """
        return 4
