...
export const useAudioTask = () => {
  ...
  const { appendAISubtitle } = useSubtitle();
  const { appendResponse, appendAIMessage } = useChatHistory();
  const { sendMessage } = useWebSocket();
  const { avatarMode } = useAvatarAppearance();
  const { autoStartMicOnConvEnd, micOn, startMic } = useVAD();
  ...
  const completingPlaybackRef = useRef(false);
  ...
  const handleAudioPlayback = (options: AudioTaskOptions): Promise<void> => new Promise((resolve) => {
    const {
      ...
      appendAISubtitle: appendSubtitle,
    ...
    if (displayText) {
      ...
      appendSubtitle(displayText.text);
      if (!forwarded) {
        sendMessage({
    ...
    try {
      ...
      if (audioBase64) {
        ...
        const shouldUseLive2DPlayback = avatarMode === 'live2d';
        let model: Live2DModel = null;

        if (shouldUseLive2DPlayback) {
          // Live2D is optional even in live2d mode.
          // If model is not ready, we still play audio without lip sync / motion.
          const live2dManager = (window as any).getLive2DManager?.();
          model = live2dManager?.getModel?.(0) ?? null;
          if (!model) {
            console.warn('Live2D model not ready, fallback to audio-only playback');
          } else if (!model._wavFileHandler) {
          ...
          if (model && lappAdapter && expressions?.[0] !== undefined) {
            setExpression(
              expressions[0],
              lappAdapter,
        ...
        audio.addEventListener('canplaythrough', () => {
          ...
          // Setup lip sync
          if (shouldUseLive2DPlayback && model && model._wavFileHandler) {
  ...
  const finalizePlaybackCompletion = useCallback(async () => {
    if (completingPlaybackRef.current) {
      return;
    }

    completingPlaybackRef.current = true;

    try {
      await audioTaskQueue.waitForCompletion();

      if (!conversationStateRef.current.backendSynthComplete) {
        return;
      }
      ...
      if (
        conversationStateRef.current.autoStartMicOnConvEnd
        && !conversationStateRef.current.micOn
      ) {
        await startMic();
      }
      ...
      completingPlaybackRef.current = false;
    }
  }, [
    sendMessage,
    setAiState,
  ...
  useEffect(() => {
    ...
    finalizePlaybackCompletion();
  }, [backendSynthComplete, finalizePlaybackCompletion]);
