...
function readPersistedLive2DScale(modelUrl?: string): number | null {
  ...
  try {
    ...
    const parsedValue = JSON.parse(rawValue) as { scale?: number };
    return typeof parsedValue?.scale === 'number' ? parsedValue.scale : null;
  } catch (error) {
    console.error('Error reading persisted Live2D scale:', error);
    return null;
  }
}

function persistLive2DScale(modelUrl: string | undefined, scale: number) {
  if (typeof window === 'undefined' || !modelUrl) {
    return;
  }
  ...
  try {
    ...
    console.error('Error saving persisted Live2D scale:', error);
  }
}
...
export const useLive2DResize = ({
  ...
  const persistedScale = readPersistedLive2DScale(modelInfo?.url);
  const initialScale = persistedScale ?? modelInfo?.kScale ?? DEFAULT_SCALE;
   ...
   */
  useEffect(() => {
    const nextPersistedScale = readPersistedLive2DScale(modelInfo?.url);
    const newInitialScale = nextPersistedScale ?? modelInfo?.kScale ?? DEFAULT_SCALE;
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    persistLive2DScale(modelInfo?.url, newTargetScale);
