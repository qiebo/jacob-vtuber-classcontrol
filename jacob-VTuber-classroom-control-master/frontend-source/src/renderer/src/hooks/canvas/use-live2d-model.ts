...
function getLive2DTransformStorageKey(modelUrl: string) {
  return `live2dTransform:${modelUrl}`;
}

function readPersistedLive2DPosition(modelUrl?: string): Position | null {
  if (typeof window === 'undefined' || !modelUrl) {
    return null;
  }

  try {
    const rawValue = window.localStorage.getItem(getLive2DTransformStorageKey(modelUrl));
    if (!rawValue) {
      return null;
    }
    const parsedValue = JSON.parse(rawValue) as { position?: Position };
    if (
      typeof parsedValue?.position?.x !== 'number'
      || typeof parsedValue?.position?.y !== 'number'
    ) {
      return null;
    }
    return parsedValue.position;
  } catch (error) {
    console.error('Error reading persisted Live2D position:', error);
    return null;
  }
}

function persistLive2DPosition(modelUrl: string | undefined, position: Position) {
  if (typeof window === 'undefined' || !modelUrl) {
    return;
  }
  ...
  try {
    ...
    window.localStorage.setItem(
      getLive2DTransformStorageKey(modelUrl),
      JSON.stringify({
        ...parsedValue,
        position,
      }),
    ...
    console.error('Error saving persisted Live2D position:', error);
  }
}
...
export const useLive2DModel = ({
  ...
  useEffect(() => {
    ...
    const liveCanvas = canvasRef.current || (document.getElementById('canvas') as HTMLCanvasElement | null);
    const canvasChanged = !!liveCanvas && trackedCanvas !== liveCanvas;
    ...
    if (needsUpdate) {
      ...
      try {
        ...
        if (baseUrl && modelDir) {
          ...
          const initializeWhenCanvasReady = (attempt = 0) => {
            const canvas = canvasRef.current || document.getElementById('canvas');
            const hasCanvasSize = canvas instanceof HTMLCanvasElement
              && canvas.clientWidth > 0
              && canvas.clientHeight > 0;

            if (!hasCanvasSize && attempt < 20) {
              window.setTimeout(() => initializeWhenCanvasReady(attempt + 1), 150);
              return;
            }
          ...
          window.setTimeout(() => initializeWhenCanvasReady(), 300);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const persistedPosition = readPersistedLive2DPosition(modelInfo?.url);
      if (persistedPosition) {
        setModelPosition(persistedPosition.x, persistedPosition.y);
        modelPositionRef.current = persistedPosition;
        modelStartPos.current = persistedPosition;
    ...
    if (isDragging) {
      ...
      if (adapter) {
        ...
        if (currentModel && currentModel._modelMatrix) {
          ...
          persistLive2DPosition(modelInfo?.url, finalPos);
        }
      }
