...
import { LAppGlManager } from "../../../WebSDK/src/lappglmanager";
...
export const Live2D = memo(
  ({ showSidebar }: Live2DProps): JSX.Element => {
    ...
    const initConfigRequestedRef = useRef(false);
    ...
    useEffect(() => {
      if (modelInfo?.url) {
        initConfigRequestedRef.current = false;
        return;
      }
      ...
      if (!initConfigRequestedRef.current) {
        initConfigRequestedRef.current = true;
        wsService.sendMessage({ type: "request-init-config" });
      }
    }, [modelInfo?.url, wsState]);
