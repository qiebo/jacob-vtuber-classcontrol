...
export const SIDEBAR_EXPANDED_WIDTH = 'clamp(660px, 68vw, 860px)';
export const SIDEBAR_EXPANDED_WIDTH_MOBILE = 'min(calc(100vw - 16px), 860px)';
const SIDEBAR_RIGHT_EXPANDED_WIDTH = 'clamp(560px, 48vw, 680px)';
const SIDEBAR_RIGHT_EXPANDED_WIDTH_MOBILE = 'min(calc(100vw - 20px), 680px)';
export const SIDEBAR_COLLAPSED_WIDTH = '0px';

const getExpandedWidth = (side: 'left' | 'right') =>
  side === 'left' ? SIDEBAR_EXPANDED_WIDTH : SIDEBAR_RIGHT_EXPANDED_WIDTH;

const getExpandedWidthMobile = (side: 'left' | 'right') =>
  side === 'left'
    ? SIDEBAR_EXPANDED_WIDTH_MOBILE
    : SIDEBAR_RIGHT_EXPANDED_WIDTH_MOBILE;
...
export const sidebarStyles = {
  sidebar: {
    container: (isCollapsed: boolean, side: 'left' | 'right' = 'left') => ({
      position: 'absolute' as const,
      [side]: 0,
      top: 0,
      height: '100%',
      width: {
        base: getExpandedWidthMobile(side),
        md: getExpandedWidth(side),
      },
      bg: 'gray.900',
      ...
      boxShadow: isCollapsed
        ? 'none'
        : side === 'left'
          ? '18px 0 38px rgba(15,23,42,0.34)'
          : '-18px 0 38px rgba(15,23,42,0.34)',
    }),
    toggleButton: (side: 'left' | 'right' = 'left', isCollapsed = false) => ({
      position: 'absolute',
      ...(side === 'left'
        ? (isCollapsed
          ? {
          ...
          : {
            left: {
              base: `calc(${getExpandedWidthMobile(side)} - 22px)`,
              md: `calc(${getExpandedWidth(side)} - 22px)`,
            },
          })
        : (isCollapsed
          ? {
