...
export function VADProvider({ children }: { children: React.ReactNode }) {
  ...
  const initVAD = async () => {
    if (typeof window !== 'undefined') {
      const isSecureOrigin = window.isSecureContext || LOOPBACK_HOSTS.has(window.location.hostname);
      if (!isSecureOrigin) {
        throw new Error(t('error.insecureMicContext'));
      }
    }
    ...
    try {
      ...
      console.warn('Failed to initialize VAD with v5, falling back to legacy model.', v5Error);
      const fallbackVAD = await MicVAD.new({
        ...sharedOptions,
        model: "legacy",
      });
      vadRef.current = fallbackVAD;
      fallbackVAD.start();
    }
  };
