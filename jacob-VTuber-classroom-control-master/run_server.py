...
def check_frontend_submodule(lang=None):
    ...
    frontend_build_path = (
        Path(__file__).parent / "frontend-source" / "dist" / "web" / "index.html"
    )
    frontend_submodule_path = Path(__file__).parent / "frontend" / "index.html"

    if frontend_build_path.exists() or frontend_submodule_path.exists():
        return

    if not frontend_submodule_path.exists():
        if lang == "zh":
        ...
        try:
            ...
            if frontend_submodule_path.exists():
                if lang == "zh":
